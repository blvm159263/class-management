...
public class TrainingClassController {
    ...
    public ResponseEntity<?> getAll(@PathVariable("id") long id) {
        return ResponseEntity.ok(trainingClassService.findClassDetail(id));
    }

    @GetMapping("/trainers")
    public ResponseEntity<?> getAllTrainers(@Param("id") long id) {
        return ResponseEntity.ok(trainingClassService.findTrainer(id));
    }

    @GetMapping("/admins")
    public ResponseEntity<?> getAllAdmins(@Param("id") long id) {
        return ResponseEntity.ok(trainingClassService.findAdmin(id));
    }
    ...
    public ResponseEntity<?> getAllTowers(@Param("id") long id) {
        return ResponseEntity.ok(trainingClassService.findTower(id));
    }

    @GetMapping("/attendee")
    public ResponseEntity<?> getAttendeeName(@Param("id") long id) {
        return ResponseEntity.ok(trainingClassService.findAttendee(id));
    }

    @GetMapping("/schedule")
    public ResponseEntity<?> getClassSchedule(@Param("id") long id) {
        return ResponseEntity.ok(trainingClassService.findClassShedule(id));
    }
